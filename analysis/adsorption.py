...
class AdsorbateSiteFinder(object):
    ...
    def mirror_adsorbates(self):
        """
        Function that mirrors all the adsorbates on one side of the slab to the matching
        sites on the other side of the slab

        author: Quinn Campbell qjc5019@psu.edu
        """
        adslab = self.slab
    ...
    def add_water_monolayer(self,distance =2.85):
        ...
        """
        slab = self.slab
        water = Molecule("HHO", [[-0.7598,0.0,-0.5841],[0.7598,0.0,-0.5841],[0,0,0]])
        min_z = 1000.0
        max_z = -10.0
        ...
        site = a*random.random() + b*random.random() + [0.0,0.0,max_z+water_distance]
        sop = SymmOp.from_origin_axis_angle(origin = [0,0,0],axis = [1,1,1] ,angle = random.random()*140.0-70.0)
        wat = water.copy()
        ...
        for i in range(wat_mol_a):
            for j in range(wat_mol_b):
                if i == 0 and j == 0:
                    ...
                    if i%2 ==1:
                        new_coord = site + float(i)/float(wat_mol_a) *a + (float(j)/float(wat_mol_b)+1.0/(float(wat_mol_b)*2.0))*b
                    else:
                        new_coord = site + float(i)/float(wat_mol_a) *a + float(j)/float(wat_mol_b)*b
                    sop = SymmOp.from_origin_axis_angle(origin = [0,0,0],axis = [1,1,1] ,angle = random.random()*140.0-70.0)
                    wat = water.copy()
                    wat.apply_operation(sop)
...
def plot_slab(slab, ax, scale=0.8, repeat=5, window=1.5,
    ...
    for n, coord in enumerate(coords):
        ...
        if any(char.isdigit() for char in sites[n].species_string):
            color = color_dict[sites[n].species_string[:-2]]
        else:
