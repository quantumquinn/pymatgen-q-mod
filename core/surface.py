...
class Slab(Structure):
    ...
    def find_surface_logK(self):

        """This method finds the logK of a surface using the MUSIC model.
               logK refers to the rxn MeO^-s + H^+ --> MeOH^{s+1} [K]

           Probably need a separate method for slabs imported from QE 

           Values for ideal distance to O come from 
              Brown, Altermatt, Acta Crystallographica, B41 244-247 (1985).
           
           Should technially have different distances for different oxidation states

        """
        #You should definitely double check any values you're concerned about. Typos can def happen
        ideal_distance_to_O = {'Ca' : {+2:1.967},
                               'Pb' : {+4: 2.042,
                                       +2: 2.112},
                               'Ag' : {+1: 1.842},
                               'Al' : {+3: 1.651},
        ...
        surf_sites = self.get_surface_sites()
        surf_O_sites = []
        for site in surf_sites['top']:
            if site[0].specie.symbol == 'O':
                surf_O_sites.append(site[0])
        #print ( surf_O_sites )


        log_K_values = []
        for site in surf_O_sites:
                
            O_neighbors = self.get_neighbors(site, r = 2.75)
            charge = -2.0
            #print ( O_neighbors )
            for neighbor in O_neighbors:
                if neighbor[0].specie.symbol != 'O':
                    #metal_neighbors = get_nearest_neighbors(slab,neighbor[0],r=2.5)
                    try:
                        charge += math.exp((ideal_distance_to_O[neighbor[0].specie.symbol][neighbor[0].specie.oxi_state]-neighbor[1])/0.37)
                    except KeyError:
                        print( "Specie %s oxidation state %s typical distance to O not in database"%(neighbor[0].specie.symbol,neighbor[0].specie.oxi_state))
                        print( "Using alternative bond valence")
                        metal_neighbors = get_nearest_neighbors(slab,neighbor[0],r=2.5)
                        charge += neighbor[0].specie.oxi_state/float(len(metal_neighbors))
                    #print( charge )
                    #print (ox[neighbor[0].specie.symbol]/float(len(metal_neighbors)))
                ...
                #print ( pK)
                #Going on the theory that no O sites is v bad and probably needs a v negative log K score
        if len(surf_O_sites) == 0:
            slab_log_K = -100.0
        else:
            slab_log_K = np.mean(log_K_values)        
...
class SlabGenerator(object):
    ...
    def get_slabs(self, bonds=None, tol=0.1, max_broken_bonds=0,
                  symmetrize=False, repair=False,force_non_polar=False): # (to be implemented...)
        """
	    ...
	    force_non_polar (bool): Whether or not to ensure that a surface is 
                non polar
        ...
        for g in m.group_structures(slabs):
            ...
            if force_non_polar:
                slabs = self.nonstoichiometric_non_polar_slab(g[0])
                new_slabs.extend(slabs)
            else:
                new_slabs.append(g[0])
    ...
    def nonstoichiometric_symmetrized_slab(self, init_slab, tol=1e-3):
        ...
        for top in [True, False]:
            ...
            while asym:
                ...
                #Makes sure that the slab does not have less atoms than the original unit cell
		#Seems reasonable enough, real question is why don't some structures appear symmetric
    ...
    def nonstoichiometric_non_polar_slab(self, init_slab, tol=1e-3):

        """
        This method checks whether the slab is non polar. If it's not, sites at the bottom of the
        slab will be removed until the slab is non polar. Note the removal of sites
        can destroy the stoichiometry of the slab. For non-elemental
        structures, the chemical potential will be needed to calculate surface energy.

        Arg:
            init_slab (Structure): A single slab structure
            tol (float): Tolerance for is_polar  .

        Returns:
            Slab (structure): A nonpolar Slab object.
        """



        if not init_slab.is_polar(tol):
            return [init_slab]
        ...
        for top in [True, False]:
            ...
            while polar:
                # Keep removing sites from the bottom one by one until both
                # surfaces are symmetric or the number of sites becomes
                # less than the number of sites in the unit cell
		...
		#Seems reasonable enough, real question is why don't some structures appear symmetric
                ...
                # Check if the altered surface is non polar
                if not slab.is_polar(tol):
                    polar = False
		    #Makes sure that the slab is still centered if that was requested
                    if self.center_slab:
                        avg_c = np.average([c[2] for c in slab.frac_coords])
                        slab.translate_sites(list(range(len(slab))), [0, 0, 0.5 - avg_c])
                    nonstoich_slabs.append(slab)
...
def generate_all_slabs(structure, max_index, min_slab_size, min_vacuum_size,
                       ...
                       include_reconstructions=False, force_non_polar=False,
                       check_highest_logK = False):
    """
    ...
    Args:
        ...
        symmetrize (bool): Whether or not to ensure the surfaces of the
            slabs are equivalent.
        force_non_polar (bool) : Whether or not to ensure the surfaces
            of the slab are non polar 
        repair (bool): Whether to repair terminations with broken bonds
            or just omit them
        include_reconstructions (bool): Whether to include reconstructed
            slabs available in the reconstructions_archive.json file.
        check_highest_logK (bool): Whether or not to only return the slab with the highest
            pK for each miller termination. pK found (very roughly) using the MUSIC model
    """
    all_slabs = []
    ...
    for miller in get_symmetrically_distinct_miller_indices(structure,
        ...
        slabs = gen.get_slabs(bonds=bonds, tol=tol, symmetrize=symmetrize,
                              ...
                              force_non_polar=force_non_polar)

        if check_highest_logK:
            max_logK = -100000.0
            for slab in slabs:
                logK = slab.find_surface_logK()
                if logK > max_logK:
                   max_logK_slab = slab
                   max_logK = logK
            slabs = []
            slabs.append(max_logK_slab)
